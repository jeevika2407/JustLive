...
aa.addEventListener('click', function (fun1) {
    ...
    bb = localStorage.getItem('Events');

    if (bb == null) {
        dd = [];
    }
    else {
        dd = JSON.parse(localStorage.getItem('Events'))
    }

    dd.push(noteswrite.value);
    localStorage.setItem('Events', JSON.stringify(dd));
    noteswrite.value = '';
    shownotes();
...
function shownotes() {
    ...
    dd.forEach(function (element, index) {
        ...
        <div class="card notescard my-2 mx-2" style="width: 12rem;">
            <div class="card-body">
                <h5 class="card-title" id="notesno"> Events ${index + 1}</h5>
                <p class="card-text">${element}</p>
                <a id="${index}" onclick="delnotes(this.id)" class="btn btn-primary">Delete Events</a>
            </div>
        </div>`;
    });
    ...
    if (dd.length != 0) {
        ...
        gg.innerHTML = '<h4>There is No Events</h4>'
    }
};
